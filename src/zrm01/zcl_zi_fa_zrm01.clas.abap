CLASS zcl_zi_fa_zrm01 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_ZI_FA_ZRM01 IMPLEMENTATION.
...
*  REPLACE ALL OCCURRENCES OF ',' IN lv_formatted WITH '.'.
  ...
  METHOD if_rap_query_provider~select.
    ...
    TYPES: BEGIN OF lty_customer_exc,
             supplieraccountgroup           TYPE string,
             ncc                            TYPE string,
             ct9_nodk_vnd                   TYPE string,
             ct10_codk_vnd                  TYPE string,
             ...
             zlevel                         TYPE char03,
             border                         TYPE char03,
           END OF lty_customer_exc.
    TYPES tt_customer_exc TYPE STANDARD TABLE OF lty_customer_exc WITH EMPTY KEY.
    ...
    TYPES: BEGIN OF lty_header,
             ...
             customersexc       TYPE tt_customer_exc,
    ...
    DATA: lt_post              TYPE TABLE OF lty_header,
          ...
          ls_customer          TYPE lty_supplier,
          ls_customer_header   TYPE lty_supplier,

          ls_customer_exc      TYPE lty_customer_exc,
          lt_customer_exc      TYPE TABLE OF  lty_customer_exc,
          lt_journal_entry_exc TYPE TABLE OF  lty_customer_exc,
    ...
    DATA:
      lt_result TYPE STANDARD TABLE OF zi_fa_zrm01,
      ls_result TYPE zi_fa_zrm01.
    DATA:
      lt_result_page TYPE STANDARD TABLE OF zi_fa_zrm01,
      lv_offset      TYPE i,
      lv_page_size   TYPE i,
...
*                    WHEN 'FLAG'.
*            DATA(lt_flag_range) = ls_filter-range.
        ENDCASE.
      ENDLOOP.
...
*         AND ( (  journalentry~isreversed IN @lt_isreversal_range AND journalentry~isreversal IN @lt_isreversal_range AND ( @lv_isreversal = '0' OR @lv_isreversal = '3' )   )
*         OR ( ( journalentry~isreversal = 'X' OR  journalentry~isreversed = 'X' ) AND @lv_isreversal = '1' ) )
        GROUP BY  customer~customer,
                  item_journalentry~companycode
         ORDER BY item_journalentry~companycode,
                  customer~customer
        INTO TABLE @DATA(lt_group_dauky).

      SELECT
        customer~customeraccountgroup && ' - ' && grouptext~accountgroupname AS customeraccountgroup,
       customer~customer,
               concat(
          concat(
    ...
    SUM(
      ...
      FROM  i_customer AS customer
        INNER JOIN i_journalentryitem AS item_journalentry ON customer~customer = item_journalentry~customer AND item_journalentry~ledger = '0L'   AND item_journalentry~financialaccounttype = 'D'
        LEFT OUTER JOIN i_journalentry AS journalentry ON journalentry~accountingdocument = item_journalentry~accountingdocument AND journalentry~fiscalyear = item_journalentry~fiscalyear AND journalentry~companycode = item_journalentry~companycode
                LEFT JOIN zcds_bp AS bp ON customer~customer = bp~businesspartner
...
SELECT
    ...
    ' - ' &&
    grouptext~accountgroupname AS supplieraccountgroup,
...
LEFT JOIN i_customercompany AS _customercompany
  ON item_journalentry~offsettingaccount = _customercompany~customer
 AND item_journalentry~companycode       = _customercompany~companycode

INNER JOIN i_journalentry AS journalentry
  ON journalentry~accountingdocument = item_journalentry~accountingdocument
 AND journalentry~fiscalyear         = item_journalentry~fiscalyear
 AND journalentry~companycode        = item_journalentry~companycode

LEFT JOIN i_journalentry AS journalentry_reversed
  ON journalentry_reversed~accountingdocument = journalentry~reversedocument
 AND journalentry_reversed~fiscalyear         = journalentry~reversedocumentfiscalyear
 AND journalentry~companycode                 = journalentry_reversed~companycode

LEFT JOIN zcds_bp AS bp
  ON customer~customer = bp~businesspartner

LEFT JOIN i_customeraccountgrouptext AS grouptext
  ON grouptext~customeraccountgroup = customer~customeraccountgroup
 AND grouptext~language = 'E'

LEFT JOIN zcds_company AS _company
  ON journalentry~companycode = _company~companycode

LEFT OUTER JOIN i_billingdocument AS bd
  ON bd~billingdocument = journalentry~originalreferencedocument

LEFT JOIN i_businesspartner
  ON i_businesspartner~businesspartner = customer~customer
...
WHERE customer~customer               IN @lt_customer_range
  ...
  AND (
    ...
    OR (
         ( journalentry~isreversal = 'X'
        OR journalentry~isreversed = 'X' )
       AND @lv_isreversal = '1'
       AND journalentry_reversed~fiscalperiod = journalentry~fiscalperiod
       )
    OR (  journalentry~isreversal IS INITIAL
        AND journalentry~isreversed IS INITIAL
       AND @lv_isreversal = '1' )
    OR (
         ( journalentry~isreversal = 'X'
        OR journalentry~isreversed = 'X' )
...
GROUP BY
    ...
    bd~yy1_customdecleration_bdh,
    bd~yy1_invoiceno_bdh,
    _company~tencty_vn,
...
*  INTO TABLE @DATA(lt_text).
        ...
        LOOP AT lt_customer_dauky ASSIGNING FIELD-SYMBOL(<fs_dauky>).
          ...
          IF sy-subrc <> 0.
            ...
            ls_result-supplieraccountgroup = <fs_dauky>-customeraccountgroup.
        ...
        LOOP AT lt_result ASSIGNING FIELD-SYMBOL(<lfs_zrm01>) .
          lv_stt = lv_stt + 1.
          <lfs_zrm01>-stt = lv_stt.
          <lfs_zrm01>-object_id = lv_uuid_fi.
          AT FIRST.
            MOVE-CORRESPONDING <lfs_zrm01> TO ls_post.
          ENDAT.

          <lfs_zrm01>-amountinbalancetransaccrcy_30 = <lfs_zrm01>-ct30_curr.
          <lfs_zrm01>-amountinbalancetransaccrcy_31 = <lfs_zrm01>-ct31_curr * -1.
          IF <lfs_zrm01>-transactioncurrency <> 'VND'.
            <lfs_zrm01>-amountinbalancetransaccrcy_32 = <lfs_zrm01>-ct32_curr.
            <lfs_zrm01>-amountinbalancetransaccrcy_33 = <lfs_zrm01>-ct33_curr * -1.
          ENDIF.


          <lfs_zrm01>-amountinbalancetransaccrcy_46  = <lfs_zrm01>-ct46_curr .
          <lfs_zrm01>-amountinbalancetransaccrcy_47  = <lfs_zrm01>-ct47_curr  * -1.


          CONDENSE <lfs_zrm01>-amountinbalancetransaccrcy_30 NO-GAPS.
          CONDENSE <lfs_zrm01>-amountinbalancetransaccrcy_31 NO-GAPS.
          CONDENSE <lfs_zrm01>-amountinbalancetransaccrcy_32 NO-GAPS.
          CONDENSE <lfs_zrm01>-amountinbalancetransaccrcy_33 NO-GAPS.
          CONDENSE <lfs_zrm01>-amountinbalancetransaccrcy_46 NO-GAPS.
          CONDENSE <lfs_zrm01>-amountinbalancetransaccrcy_47 NO-GAPS.

          <lfs_zrm01>-amountinbalancetransaccrcy_30 = format_amount_string( i_currency = CONV string( <lfs_zrm01>-transactioncurrency ) i_input = CONV string( <lfs_zrm01>-amountinbalancetransaccrcy_30 ) ).
          <lfs_zrm01>-amountinbalancetransaccrcy_31 = format_amount_string( i_currency = CONV string( <lfs_zrm01>-transactioncurrency ) i_input = CONV string( <lfs_zrm01>-amountinbalancetransaccrcy_31 ) ).
          IF <lfs_zrm01>-transactioncurrency <> 'VND'.
            <lfs_zrm01>-amountinbalancetransaccrcy_32 = format_amount_string( i_currency = CONV string( <lfs_zrm01>-transactioncurrency ) i_input = CONV string( <lfs_zrm01>-amountinbalancetransaccrcy_32 ) ).
            <lfs_zrm01>-amountinbalancetransaccrcy_33 = format_amount_string( i_currency = CONV string( <lfs_zrm01>-transactioncurrency ) i_input = CONV string( <lfs_zrm01>-amountinbalancetransaccrcy_33 ) ).
          ENDIF.
          <lfs_zrm01>-amountinbalancetransaccrcy_46 = format_amount_string( i_currency = CONV string( 'VND' ) i_input = CONV string( <lfs_zrm01>-amountinbalancetransaccrcy_46 ) ).
          <lfs_zrm01>-amountinbalancetransaccrcy_47 = format_amount_string( i_currency = CONV string( 'VND' ) i_input = CONV string( <lfs_zrm01>-amountinbalancetransaccrcy_47 ) ).


* Start - Get data lt_journal_entrys
          MOVE-CORRESPONDING <lfs_zrm01> TO ls_journal_entry.
          IF <lfs_zrm01>-taxexchangerate = 0.
          ...
          IF <lfs_zrm01>-accountingdocumenttype =  'RV'.
            ...
            IF sy-subrc = 0.
              ls_journal_entry-accountingdocumentheadertext = <lfs_text>-billingdocumentitemtext.
            ENDIF.
          ENDIF.
          IF <lfs_zrm01>-documentdate IS NOT INITIAL.
            ls_journal_entry-documentdate =
              |{ <lfs_zrm01>-documentdate+6(2) }/{ <lfs_zrm01>-documentdate+4(2) }/{ <lfs_zrm01>-documentdate+0(4) }|.
          ENDIF.

          IF <lfs_zrm01>-postingdate IS NOT INITIAL.
            ls_journal_entry-postingdate =
              |{ <lfs_zrm01>-postingdate+6(2) }/{ <lfs_zrm01>-postingdate+4(2) }/{ <lfs_zrm01>-postingdate+0(4) }|.
          ENDIF.
          ls_journal_entry-journalentrylastchangedatetime = ls_journal_entry-documentdate.
          ls_journal_entry-glaccount_27 = <lfs_zrm01>-glaccount.

          ls_customer-ct13_cpsn_vnd   = ls_customer-ct13_cpsn_vnd + <lfs_zrm01>-ct30_curr.
          ls_customer-ct14_cpsc_vnd   = ls_customer-ct14_cpsc_vnd + <lfs_zrm01>-ct31_curr * -1.
          ls_customer-ct15_cpsn       = ls_customer-ct15_cpsn + <lfs_zrm01>-ct32_curr.
          ls_customer-ct16_cpsc       = ls_customer-ct16_cpsc + <lfs_zrm01>-ct33_curr * -1.
          ls_customer-ct44_cpsn       = ls_customer-ct44_cpsn + <lfs_zrm01>-ct46_curr .
          ls_customer-ct45_cpsc       = ls_customer-ct45_cpsc + <lfs_zrm01>-ct47_curr  * -1.


          IF <lfs_zrm01>-accountingdocument IS NOT INITIAL.
            APPEND ls_journal_entry TO lt_journal_entrys.
            "ViHT6/16.04.2026/Update ZRM01 Excel
            MOVE-CORRESPONDING ls_journal_entry TO ls_customer_exc.
            ls_customer_exc-zlevel = 'B'.
            APPEND ls_customer_exc TO lt_journal_entry_exc.
            CLEAR: ls_customer_exc.
            "ViHT6/16.04.2026/Update ZRM01 Excel
          ENDIF.
...
* Start - Get data lt_suppliers
          ...
          AT END OF supplier.
            MOVE-CORRESPONDING <lfs_zrm01> TO ls_customer_header.

            READ TABLE lt_group_dauky ASSIGNING FIELD-SYMBOL(<lfs_dauky>) WITH KEY  companycode = <lfs_zrm01>-companycode
                                                                                         customer = <lfs_zrm01>-supplier BINARY SEARCH.
*                                                                                      glaccount = <lfs_zrm01>-glaccount_26 BINARY SEARCH.
            IF sy-subrc = 0.

              IF <lfs_dauky>-dauky_vnd >= 0.
                ls_customer_header-ct9_nodk_vnd = <lfs_dauky>-dauky_vnd .
              ENDIF.
              IF <lfs_dauky>-dauky_vnd < 0.

                ls_customer_header-ct10_codk_vnd =  abs( <lfs_dauky>-dauky_vnd ).
              ENDIF.
              IF <lfs_dauky>-dauky_nt_vnd >= 0.

                ls_customer_header-ct11_nodk = <lfs_dauky>-dauky_nt.
              ENDIF.
              IF <lfs_dauky>-dauky_nt_vnd < 0.

                ls_customer_header-ct12_codk = abs( <lfs_dauky>-dauky_nt ).
              ENDIF.

              IF <lfs_dauky>-dauky >= 0.
                ls_customer_header-ct44_nodk =  <lfs_dauky>-dauky.
              ELSE.
                ls_customer_header-ct45_codk = abs( <lfs_dauky>-dauky ).
              ENDIF.
              ls_post-sum_ct9   = ls_post-sum_ct9 + ls_customer_header-ct9_nodk_vnd.
              ls_post-sum_ct10  = ls_post-sum_ct10 + ls_customer_header-ct10_codk_vnd.
              ls_post-sum_ct11  = ls_post-sum_ct11 + ls_customer_header-ct11_nodk.
              ls_post-sum_ct12  = ls_post-sum_ct12 + ls_customer_header-ct12_codk.

              ls_post-sum_nosddk  = ls_post-sum_nosddk + ls_customer_header-ct44_nodk.
              ls_post-sum_cosddk  = ls_post-sum_cosddk + ls_customer_header-ct45_codk.

            ENDIF.

            IF <lfs_zrm01>-accountingdocument IS INITIAL.
              ls_customer_header-ct9_nodk_vnd = abs( <lfs_zrm01>-ct9_nodk_vnd ).
              ls_customer_header-ct10_codk_vnd = abs( <lfs_zrm01>-ct10_codk_vnd ).
              ls_customer_header-ct11_nodk = abs( <lfs_zrm01>-ct11_nodk ).
              ls_customer_header-ct12_codk = abs( <lfs_zrm01>-ct12_codk ).

              ls_customer_header-ct44_nodk = <lfs_zrm01>-ct44_nodk .
              ls_customer_header-ct45_codk =  abs( <lfs_zrm01>-ct45_codk ) .
            ...
            ls_customer_header-ct13_cpsn_vnd = ls_customer-ct13_cpsn_vnd.
            ls_customer_header-ct14_cpsc_vnd = ls_customer-ct14_cpsc_vnd.
            ls_customer_header-ct15_cpsn = ls_customer-ct15_cpsn.
            ls_customer_header-ct16_cpsc = ls_customer-ct16_cpsc.

            ls_customer_header-ct44_cpsn = ls_customer-ct44_cpsn.
            ls_customer_header-ct45_cpsc = ls_customer-ct45_cpsc.

            IF ( abs( ls_customer_header-ct44_nodk ) - abs( ls_customer_header-ct45_codk )
                 + abs( ls_customer-ct44_cpsn ) - abs( ls_customer-ct45_cpsc ) ) > 0.
              ls_customer_header-ct45_sdnck =  abs( ls_customer_header-ct44_nodk ) - abs( ls_customer_header-ct45_codk )
                                               + abs( ls_customer-ct44_cpsn ) - abs( ls_customer-ct45_cpsc ).
            ELSE.
              ls_customer_header-ct45_sdcck = abs( abs( ls_customer_header-ct44_nodk ) - abs( ls_customer_header-ct45_codk )
                                              + abs( ls_customer-ct44_cpsn ) - abs( ls_customer-ct45_cpsc ) ).
            ENDIF.

            IF ( abs( ls_customer_header-ct9_nodk_vnd ) - abs( ls_customer_header-ct10_codk_vnd )
                 + abs( ls_customer-ct13_cpsn_vnd ) - abs( ls_customer-ct14_cpsc_vnd ) ) > 0.
              ls_customer_header-ct17_sdnck_vnd = abs( ls_customer_header-ct9_nodk_vnd ) - abs( ls_customer_header-ct10_codk_vnd )
                                                  + abs( ls_customer-ct13_cpsn_vnd ) - abs( ls_customer-ct14_cpsc_vnd ).
            ELSE.
              ls_customer_header-ct18_sdcck_vnd = abs(  abs( ls_customer_header-ct9_nodk_vnd ) - abs( ls_customer_header-ct10_codk_vnd )
                                                  + abs( ls_customer-ct13_cpsn_vnd ) - abs( ls_customer-ct14_cpsc_vnd ) ).
            ENDIF.

            IF ( abs( ls_customer_header-ct11_nodk ) - abs( ls_customer_header-ct12_codk )
                 + abs( ls_customer-ct15_cpsn ) - abs( ls_customer-ct16_cpsc ) ) > 0.
              ls_customer_header-ct19_sdnck = abs( ls_customer_header-ct11_nodk ) - abs( ls_customer_header-ct12_codk )
                                              + abs( ls_customer-ct15_cpsn ) - abs( ls_customer-ct16_cpsc ).
            ELSE.
              ls_customer_header-ct20_sdcck = abs( abs( ls_customer_header-ct11_nodk ) - abs( ls_customer_header-ct12_codk )
                                              + abs( ls_customer-ct15_cpsn ) - abs( ls_customer-ct16_cpsc ) ).
            ENDIF.

            ls_post-sum_cpsn_vnd = ls_post-sum_cpsn_vnd + ls_customer_header-ct13_cpsn_vnd.
            ls_post-sum_cpsc_vnd = ls_post-sum_cpsc_vnd + ls_customer_header-ct14_cpsc_vnd.
            ls_post-sum_cpsn = ls_post-sum_cpsn + ls_customer_header-ct15_cpsn.
            ls_post-sum_cpsc = ls_post-sum_cpsc + ls_customer_header-ct16_cpsc.
            ls_post-sum_sdnck_vnd = ls_post-sum_sdnck_vnd +
                    ( abs( ls_customer_header-ct9_nodk_vnd )
                    - abs( ls_customer_header-ct10_codk_vnd )
                    + abs( ls_customer-ct13_cpsn_vnd )
                    - abs( ls_customer-ct14_cpsc_vnd ) ).

            ls_post-sum_sdnck = ls_post-sum_sdnck +
                ( abs( ls_customer_header-ct11_nodk )
                - abs( ls_customer_header-ct12_codk )
                + abs( ls_customer-ct15_cpsn )
                - abs( ls_customer-ct16_cpsc ) ).

            ls_post-sum_cpsn_lk = ls_post-sum_cpsn_lk + ls_customer_header-ct44_cpsn.
            ls_post-sum_cpsc_lk  = ls_post-sum_cpsc_lk + ls_customer_header-ct45_cpsc.

            ls_post-sum_sdnck_lk = ls_post-sum_sdnck_lk +
                 ( abs( ls_customer_header-ct44_nodk )
                 - abs( ls_customer_header-ct45_codk )
                 + abs( ls_customer-ct44_cpsn )
                 - abs( ls_customer-ct45_cpsc ) ).

            ls_customer-ct13_cpsn_vnd = format_amount_string( i_currency = 'VND' i_input = CONV string( ls_customer-ct13_cpsn_vnd ) ).
            ls_customer-ct14_cpsc_vnd = format_amount_string( i_currency = 'VND' i_input = CONV string( ls_customer-ct14_cpsc_vnd ) ).
            ls_customer-ct15_cpsn     = format_amount_string( i_currency = 'USD' i_input = CONV string( ls_customer-ct15_cpsn ) ).
            ls_customer-ct16_cpsc     = format_amount_string( i_currency = 'USD' i_input = CONV string( ls_customer-ct16_cpsc ) ).

            ls_customer_header-ct9_nodk_vnd  = format_amount_string( i_currency = 'VND' i_input = CONV string( ls_customer_header-ct9_nodk_vnd ) ).
            ls_customer_header-ct10_codk_vnd = format_amount_string( i_currency = 'VND' i_input = CONV string( ls_customer_header-ct10_codk_vnd ) ).
            ls_customer_header-ct11_nodk     = format_amount_string( i_currency = 'USD' i_input = CONV string( ls_customer_header-ct11_nodk ) ).
            ls_customer_header-ct12_codk     = format_amount_string( i_currency = 'USD' i_input = CONV string( ls_customer_header-ct12_codk ) ).

            ls_customer_header-ct13_cpsn_vnd = format_amount_string( i_currency = 'VND' i_input = CONV string( ls_customer_header-ct13_cpsn_vnd ) ).
            ls_customer_header-ct14_cpsc_vnd = format_amount_string( i_currency = 'VND' i_input = CONV string( ls_customer_header-ct14_cpsc_vnd ) ).
            ls_customer_header-ct15_cpsn     = format_amount_string( i_currency = 'USD' i_input = CONV string( ls_customer_header-ct15_cpsn ) ).
            ls_customer_header-ct16_cpsc     = format_amount_string( i_currency = 'USD' i_input = CONV string( ls_customer_header-ct16_cpsc ) ).
            ls_customer_header-ct17_sdnck_vnd = format_amount_string( i_currency = 'VND' i_input = CONV string( ls_customer_header-ct17_sdnck_vnd ) ).
            ls_customer_header-ct18_sdcck_vnd = format_amount_string( i_currency = 'VND' i_input = CONV string( ls_customer_header-ct18_sdcck_vnd ) ).
            ls_customer_header-ct19_sdnck     = format_amount_string( i_currency = 'USD' i_input = CONV string( ls_customer_header-ct19_sdnck ) ).
            ls_customer_header-ct20_sdcck     = format_amount_string( i_currency = 'USD' i_input = CONV string( ls_customer_header-ct20_sdcck ) ).

            ls_customer_header-ct44_nodk = format_amount_string( i_currency = 'VND' i_input = CONV string( ls_customer_header-ct44_nodk ) ).
            ls_customer_header-ct45_codk = format_amount_string( i_currency = 'VND' i_input = CONV string( ls_customer_header-ct45_codk ) ).

            ls_customer_header-ct44_cpsn = format_amount_string( i_currency = 'VND' i_input = CONV string( ls_customer_header-ct44_cpsn ) ).
            ls_customer_header-ct45_cpsc = format_amount_string( i_currency = 'VND' i_input = CONV string( ls_customer_header-ct45_cpsc ) ).

            ls_customer_header-ct45_sdnck = format_amount_string( i_currency = 'VND' i_input = CONV string( ls_customer_header-ct45_sdnck ) ).
            ls_customer_header-ct45_sdcck = format_amount_string( i_currency = 'VND' i_input = CONV string( ls_customer_header-ct45_sdcck ) ).


            ls_customer_header-journal_entrys = lt_journal_entrys.
            APPEND ls_customer_header TO lt_suppliers.

            "ViHT6/16.04.2026/Update ZRM01 Excel
            MOVE-CORRESPONDING ls_customer_header TO ls_customer_exc.
            ls_customer_exc-zlevel = 'A'.
            APPEND ls_customer_exc TO lt_customer_exc.
            ls_customer_exc-zlevel = 'C'.
            ls_customer_exc-ncc = 'Khách hàng:' && ls_customer_exc-ncc.
            APPEND ls_customer_exc TO lt_customer_exc.


            ls_customer_exc-sddk_h  = 'Số dư đầu kỳ'.
            ls_customer_exc-zlevel = 'D'.
            APPEND ls_customer_exc TO lt_customer_exc.

            ls_customer_exc-cps_h   = 'Cộng phát sinh'.
            ls_customer_exc-zlevel = 'E'.
            APPEND ls_customer_exc TO lt_customer_exc.

            ls_customer_exc-csdck_h = 'Số dư cuối kỳ'.
            ls_customer_exc-zlevel = 'F'.
            APPEND ls_customer_exc TO lt_customer_exc.


            APPEND LINES OF lt_journal_entry_exc TO lt_customer_exc.
            "ViHT6/16.04.2026/Update ZRM01 Excel
            CLEAR: ls_customer,ls_customer_header,lt_journal_entrys,lt_journal_entry_exc.
          ENDAT.
   ...
   AND lt_glaccount_range[ 1 ]-sign = 'I'
     AND lt_glaccount_range[ 1 ]-option = 'EQ' .
       ...
       AND lt_glaccount_range[ 1 ]-sign = 'I'
        ...
        ls_post-customersexc = lt_customer_exc.
...
*              iv_form_name     = 'ZFA_F_ZRM01'
*              iv_template_name = 'ZFA_F_ZRM01'
*            ).
            DATA(ls_template) = lo_store->get_template_by_tcode( iv_tcode = 'ZRM01' ).
        ...
        LOOP AT ls_post-suppliers INTO DATA(ls_supplierxml).
          ...
          lv_xml = |{ lv_xml }<ncc>Khách hàng: { escape( val = ls_supplierxml-ncc format = cl_abap_format=>e_xml_text ) }</ncc>|.
          ...
          LOOP AT ls_supplierxml-journal_entrys INTO DATA(ls_entry).
            ...
            lv_xml = |{ lv_xml }<Z_TOKHAI>{ ls_entry-z_tokhai }</Z_TOKHAI>|.
            lv_xml = |{ lv_xml }<Z_INVOICENO>{ ls_entry-z_invoiceno }</Z_INVOICENO>|.
        ...
        DATA: lv_attacment_exc TYPE zde_attachment,
              lv_report        TYPE char72 VALUE 'ZRM01',
              lv_template      TYPE char72 VALUE 'ZRM01_EXC'.

        ls_post-report_name = 'SỔ CHI TIẾT CÔNG NỢ PHẢI THU KHÁCH HÀNG'.
...
*        ENDIF.
        ...
        DATA(lo_saver) = NEW zcl_save_pdf_zrm01( ).

        lo_saver->save_pdf(
          iv_reportid = 'ZFA_ZRM01'
