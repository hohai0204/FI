CLASS zcl_save_exc_zrm02 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    METHODS save_exc_zrm02
      IMPORTING
        iv_objectid TYPE ztb_exc_zrm02-object_id
        iv_reportid TYPE ztb_exc_zrm02-report_id
        iv_item     TYPE tt_zrm02_lines.
  PROTECTED SECTION.
  PRIVATE SECTION.
ENDCLASS.
...
CLASS ZCL_SAVE_EXC_ZRM02 IMPLEMENTATION.


  METHOD save_exc_zrm02.
    DATA: lt_db    TYPE TABLE OF ztb_exc_zrm02,
          ls_db    TYPE ztb_exc_zrm02,
          ls_input TYPE ty_lines.

*    DELETE FROM ztb_exc_zrm02 WHERE report_id = 'ZRM02'.

    DATA: lv_date_after TYPE datum.
    lv_date_after = syst-datum - 1.
    DELETE FROM ztb_exc_zrm02 WHERE create_date < @lv_date_after OR ( create_date = @lv_date_after AND create_time < @syst-uzeit ).

    LOOP AT iv_item INTO ls_input.
      CLEAR: ls_db.
      MOVE-CORRESPONDING ls_input-items TO ls_db.
      ls_db-customer = |{ ls_db-customer ALPHA = OUT }|.
    ...
    INSERT ztb_exc_zrm02 FROM TABLE @lt_db.
    IF sy-subrc <> 0.
      " Có thể update nếu đã tồn tại
      UPDATE ztb_exc_zrm02 FROM TABLE @lt_db.
    ENDIF.
  ENDMETHOD.
ENDCLASS.
