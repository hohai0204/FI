CLASS zcl_zpm02_exc_lines_impl DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES: tt_data_output TYPE STANDARD TABLE OF zfa_i_zpm02_exc_lines.
  PROTECTED SECTION.
  PRIVATE SECTION.
...
CLASS ZCL_ZPM02_EXC_LINES_IMPL IMPLEMENTATION.


  METHOD get_data.
    DATA lt_data_output TYPE tt_data_output.
    DATA: lt_lines TYPE TABLE OF zfa_i_zpm02_exc_lines,
          ls_lines TYPE zfa_i_zpm02_exc_lines.
    " Sort
    ...
    IF lt_range IS NOT INITIAL.
      LOOP AT lt_range INTO DATA(ls_range).
        CASE ls_range-name.
          ...
          WHEN 'SUPPLIERCODE'.
*            DATA(lr_supplier_range) = ls_range-range.
    ...
    SELECT view_entity~companycode,
        ...
        view_entity~glaccountlongname
      FROM zfa_i_zpm02n_view AS view_entity
      INNER JOIN i_journalentryitem AS journalentryitem ON view_entity~companycode = journalentryitem~companycode "AND view_entity~fiscalyear = journalentryitem~fiscalyear
                                                       AND view_entity~glaccount = journalentryitem~glaccount
                                                       AND journalentryitem~ledger               =  view_entity~ledger
                                                       AND journalentryitem~supplier               =  view_entity~supplier
      WHERE journalentryitem~balancetransactioncurrency IN @lr_cuki_range
...
**    "------------------------ NaVTT/ 03.04.2026/ Đầu kì check reversed -----------------------"
    ...
    INNER JOIN i_journalentry AS journalentry         ON journalentryitem~companycode        = journalentry~companycode
                                                     ...
                                                     AND journalentryitem~accountingdocument = journalentry~accountingdocument
    WHERE journalentryitem~postingdate < @lv_prev_postingdate
*      AND journalentryitem~fiscalyear IN @lr_fiscalyear_range ViHT9/14.01.2026/ update ZPM02_EXC ver 2.3.3
      AND journalentryitem~balancetransactioncurrency IN @lr_cuki_range
...
*       APPENDING TABLE @lt_dk.
    ...
    SELECT gl_account~supplieraccountgroup, gl_account~supplier, gl_account~suppliername, gl_account~glaccount, journalentryitem~postingdate,
           ...
           INNER JOIN i_journalentryitem AS journalentryitem ON gl_account~companycode       = journalentryitem~companycode
            ...
            GROUP BY gl_account~supplieraccountgroup, gl_account~supplier, gl_account~suppliername, gl_account~glaccount, journalentryitem~balancetransactioncurrency, journalentryitem~postingdate,
              journalentryitem~isreversal, journalentryitem~debitcreditcode, journalentryitem~companycodecurrency, journalentryitem~accountingdocument, journalentryitem~reversalreferencedocument, journalentryitem~fiscalperiod
    ...
    GROUP BY gl_account~supplieraccountgroup, gl_account~supplier, gl_account~suppliername, gl_account~glaccount, journalentryitem~balancetransactioncurrency, journalentryitem~postingdate,
    journalentryitem~isreversal, journalentryitem~debitcreditcode, journalentryitem~companycodecurrency, journalentryitem~accountingdocument, journalentryitem~reversalreferencedocument, journalentryitem~fiscalperiod
    ORDER BY gl_account~supplier, gl_account~glaccount, journalentryitem~balancetransactioncurrency, journalentryitem~accountingdocument
    ...
    GROUP BY gl_account~supplieraccountgroup, gl_account~supplier, gl_account~suppliername, gl_account~glaccount, journalentryitem~balancetransactioncurrency, journalentryitem~postingdate,
    journalentryitem~isreversal, journalentryitem~debitcreditcode, journalentryitem~companycodecurrency, journalentryitem~accountingdocument, journalentryitem~reversalreferencedocument, journalentryitem~fiscalperiod
    ORDER BY gl_account~supplier, gl_account~glaccount, journalentryitem~balancetransactioncurrency, journalentryitem~reversalreferencedocument
    ...
    SELECT gl_account~supplieraccountgroup, gl_account~supplier, gl_account~suppliername, gl_account~glaccount,
     ...
     INNER JOIN i_journalentryitem AS journalentryitem ON gl_account~companycode       = journalentryitem~companycode
      ...
      WHERE journalentryitem~postingdate IN @lr_postingdate_range
        ...
        AND journalentryitem~balancetransactioncurrency IN @lr_cuki_range
        AND journalentryitem~ledger = '0L'
         AND ( journalentryitem~financialaccounttype = 'K' OR ( journalentryitem~accountingdocumenttype = 'SA' AND journalentryitem~supplier IS NOT INITIAL ) ) " ViHT9/14.01.2026/ update ZPM02_EXC ver 2.3.3
...
*      ENDAT.
      ...
      READ TABLE lt_sodu_vn TRANSPORTING NO FIELDS WITH KEY supplieraccountgroup = <lfs_glaccount>-supplieraccountgroup supplier = <lfs_glaccount>-supplier glaccount = <lfs_glaccount>-glaccount BINARY SEARCH.
      IF sy-subrc = 0.
        LOOP AT lt_sodu_vn INTO DATA(ls_sd_vn) FROM sy-tabix.
          IF ls_sd_vn-supplieraccountgroup = <lfs_glaccount>-supplieraccountgroup AND ls_sd_vn-supplier = <lfs_glaccount>-supplier AND ls_sd_vn-glaccount = <lfs_glaccount>-glaccount.
...
*            ls_lines-companycodecurrency = ls_sd_vn-companycodecurrency.
        ...
        ls_lines-suppliercode = |{ ls_lines-supplier ALPHA = OUT }|.
        ls_lines-postingdate = lv_prev_postingdate. "--> Note: fill data temp tránh lỗi hệ thống sai data type DATS null
        ...
        ls_lines-cuoiki_co_vn   = lv_cuoiki_co_vn.
...
*        ls_lines-title_type = 7.
      ...
      READ TABLE lt_sodu_nt TRANSPORTING NO FIELDS WITH KEY supplieraccountgroup = <lfs_glaccount>-supplieraccountgroup supplier = <lfs_glaccount>-supplier glaccount = <lfs_glaccount>-glaccount BINARY SEARCH.
      IF sy-subrc = 0.
        LOOP AT lt_sodu_nt INTO DATA(ls_sd_nt) FROM sy-tabix.
          IF ls_sd_nt-supplieraccountgroup = <lfs_glaccount>-supplieraccountgroup AND ls_sd_nt-supplier = <lfs_glaccount>-supplier AND ls_sd_nt-glaccount = <lfs_glaccount>-glaccount.
...
*        ls_lines-title_type = 9.
        ls_lines-postingdate = lv_prev_postingdate. "--> Note: fill data temp tránh lỗi hệ thống sai data type DATS null
        IF <lfs_glaccount>-supplieraccountgroup = 'Z10'.
          ls_lines-title = |Tổng nhóm trong nước|.
        ELSEIF <lfs_glaccount>-supplieraccountgroup = 'Z20'.
          ls_lines-title = |Tổng nhóm nước ngoài|.
        ELSEIF <lfs_glaccount>-supplieraccountgroup = 'Z30'.
          ls_lines-title = |Tổng nhóm NH, HQ, CQNN|.
        ELSEIF <lfs_glaccount>-supplieraccountgroup = 'Z40'.
          ls_lines-title = |Tổng nhóm nội bộ|.
        ELSEIF <lfs_glaccount>-supplieraccountgroup = 'Z50'.
...
*        ls_lines-title_type = 10.
        ...
        ls_lines-balancetransactioncurrency = 'USD'.
        ls_lines-companycodecurrency = 'VND'.
        ls_lines-dauki_co_nt    = abs( lv_total_dk_co_nt ).
        ls_lines-dauki_no_nt    = abs( lv_total_dk_no_nt ).

        ls_lines-phatsinh_co_nt = abs( lv_total_ps_co_nt ).
        ls_lines-phatsinh_no_nt = abs( lv_total_ps_no_nt ).

        ls_lines-cuoiki_co_nt = abs( lv_total_ck_co_nt ).
        ls_lines-cuoiki_no_nt = abs( lv_total_ck_no_nt ).

*        IF ( lv_total_ck_co_nt + lv_total_ck_no_nt ) < 0.
*          ls_lines-cuoiki_co_nt   = abs( lv_total_ck_co_nt + lv_total_ck_no_nt ) .
*        ELSE.
*          ls_lines-cuoiki_no_nt   = ( lv_total_ck_co_nt + lv_total_ck_no_nt ) .
*        ENDIF.


        ls_lines-dauki_co_vn    = abs( lv_total_dk_co_vn ).
        ls_lines-dauki_no_vn    = abs( lv_total_dk_no_vn ).

        ls_lines-phatsinh_co_vn = abs( lv_total_ps_co_vn ).
        ls_lines-phatsinh_no_vn = abs( lv_total_ps_no_vn ).

        ls_lines-cuoiki_co_vn = abs( lv_total_ck_co_vn ).
        ls_lines-cuoiki_no_vn = abs( lv_total_ck_no_vn ).

*        IF ( lv_total_ck_co_vn + lv_total_ck_no_vn ) < 0.
*          ls_lines-cuoiki_co_vn   = abs( ( lv_total_ck_co_vn + lv_total_ck_no_vn ) ).
*        ELSE.
*          ls_lines-cuoiki_no_vn   = ( lv_total_ck_co_vn + lv_total_ck_no_vn ).
*        ENDIF.
