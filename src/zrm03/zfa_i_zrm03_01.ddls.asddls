...
define view entity ZFA_I_ZRM03_01
  ...
  //                                                           and fi_item.TaxCode              is not initial
    ...
    and linetax.CompanyCode = Fi_head.CompanyCode and linetax.FiscalYear = Fi_head.FiscalYear 
left outer join ZI_FA_ITEMBILLING_FIDOC as ItemBill on  ItemBill.AccountingDocument = Fi_head.AccountingDocument and ItemBill.CompanyCode = Fi_head.CompanyCode and ItemBill.FiscalYear = Fi_head.FiscalYear
...
and REv01_r.ReverseFiscalYear = Fi_head.FiscalYear
 left outer join      I_TaxCodeRate                           on  I_TaxCodeRate.TaxCode                 = fi_item.TaxCode
                                                            and I_TaxCodeRate.Country                 = 'VN'
                                                            and I_TaxCodeRate.TaxCalculationProcedure = '0TXVN'
                                                            and I_TaxCodeRate.CndnRecordValidityStartDate <= $session.system_date 
  and I_TaxCodeRate.CndnRecordValidityEndDate >= $session.system_date 
{
    ...
    concat( Fi_head.CompanyCode, Fi_head.AccountingDocument)
     , _Tax_Group.tax_group ), ItemTax.TaxCode )                                                                as Object,
      ...
      Customer_fi.ADDRESS      end
       ...
       case when Fi_head.AccountingDocumentType = 'RV' then Customer.MST    else    Customer_fi.MST end       as MST_Customer,
      Fi_head.DocumentReferenceID,
      Fi_head.DocumentDate,
    
      case when REv01.ReverseDocument is not initial then REv01.ReverseDocument
          when REv01_r.ReverseDocument  is not initial then REv01_r.AccountingDocument  end as ReverseDocument,
  ...
  //    fi_item.AssignmentReference                                                                               as AssignmentReference,
       substring(
      Fi_head.DocumentReferenceID,1,1 )   as AssignmentReference,
     case 
     when ItemBill.Text is not initial then ItemBill.Text
     when item_KD.DocumentItemText is not initial or  fi_item.DocumentItemText is not initial
     then
      case   when linetax.Count_taxcode = 1 then item_KD.DocumentItemText else  fi_item.DocumentItemText    end 
      else Fi_head.AccountingDocumentHeaderText end
      as itemtext,
      fi_item.AccountingDocumentItem                                                                            as acc_item,
      substring(
      Fi_head.DocumentReferenceID,2,
      instr( Fi_head.DocumentReferenceID, '.' ) - 2
                                             )                                                                  as Pattern,
      substring(
      Fi_head.DocumentReferenceID,
      ...
      ( ItemTax.BaseAmount   * -1 )                                                                                     as amount,
      ...
      @Semantics.amount.currencyCode: 'CompanyCodeCurrency'
      cast( ( ItemTax.TaxAmount * -1 )  as abap.curr( 17, 2 )    )                                                       as Taxamount,
  //    ItemTax.TaxRate,
  ...
  ( ItemTax.TaxBaseAmountInTransCrcy  * -1 )   as TaxBaseAmountInTransCrcy,
   @Semantics.amount.currencyCode: 'TransactionCurrency'
   ( ItemTax.TAXAMOUNTTRANS * -1 )   as TAXAMOUNTTRANS,
  @Semantics.amount.currencyCode: 'TransactionCurrency'
  ( ItemTax.TaxBaseAmountInTransCrcy + ItemTax.TAXAMOUNTTRANS ) * -1 as amount_NT,
