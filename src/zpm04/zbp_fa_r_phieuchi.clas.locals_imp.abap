CLASS lhc_zfa_r_phieuchi DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.

    METHODS get_instance_features FOR INSTANCE FEATURES
      IMPORTING keys REQUEST requested_features FOR zfa_r_phieuchi RESULT result.

    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
      IMPORTING keys REQUEST requested_authorizations FOR zfa_r_phieuchi RESULT result.

    METHODS printpdf FOR MODIFY
      IMPORTING keys FOR ACTION zfa_r_phieuchi~printpdf RESULT result.

    METHODS getdefaultsforsign FOR READ
      IMPORTING keys FOR FUNCTION zfa_r_phieuchi~GetDefaultsForSign RESULT result.

ENDCLASS.

CLASS lhc_zfa_r_phieuchi IMPLEMENTATION.
  ...
  METHOD printpdf.
    READ ENTITIES OF zfa_r_phieuchi IN LOCAL MODE
     ENTITY zfa_r_phieuchi
...
*      iv_form_name     = 'ZFI_PM04'
*      iv_template_name = 'ZFI_PM04'
*    ).
 DATA(ls_template) = lo_store->get_template_by_tcode( iv_tcode = 'ZPM04' ).
          ##no_handler
...
*    lv_number = abs( ls_xml-Amountcredit * 100 ).
*    ELSE.
*      lv_number = abs( ls_xml-Amountcredit ).
*ENDIF.
*      lv_number = lv_number .
...
IF ls_xml-curency = 'VND'.
     zcl_convert_amount_to_word=>excute(
        EXPORTING
          iv_amount   = CONV #( ls_xml-Amountcredit )
...
lv_name = ls_xml-customername.
lv_address = ls_xml-CustomerAdress.
" Normalize tiếng Việt trước khi escape XML
lv_name = zcl_unicode_util=>normalize_vietnamese( lv_name ).
lv_address = zcl_unicode_util=>normalize_vietnamese( lv_address ).
      lv_date = |Ngày { ls_xml-postingdate+6(2) } tháng { ls_xml-postingdate+4(2) } Năm { ls_xml-postingdate+0(4) }|.
      ...
      lv_xml = |{ lv_xml }<CUSTOMERNAME>{  escape( val =  lv_name format = cl_abap_format=>e_xml_text )  } </CUSTOMERNAME>|.
      lv_xml = |{ lv_xml }<CUSTOMERADRESS>{  escape( val = lv_address format = cl_abap_format=>e_xml_text ) }</CUSTOMERADRESS>|.
      lv_xml = |{ lv_xml }<REASON>{ ls_xml-reason }</REASON>|.
      lv_xml = |{ lv_xml }<AMOUNTCREDIT>{ ls_xml-Amountcredit }</AMOUNTCREDIT>|.
      ...
      lv_xml = |{ lv_xml }<AMOUNTCREDIT_VND>{ ls_xml-Amountcredit_VND }</AMOUNTCREDIT_VND>|.
...
*      INTO @DATA(lv_exsist).
      SELECT SINGLE object_id FROM ztb_fi_pdf_draf WHERE object_id = @ls_xml-object AND report_id = 'ZFI_PM04'
      INTO @DATA(lv_exsist).
  ...
  METHOD GetDefaultsForSign.
      ...
      ENTITY zfa_r_phieuchi
      ALL FIELDS WITH CORRESPONDING #( keys )
      RESULT DATA(lt_phieuchi).

    CHECK lt_phieuchi IS NOT INITIAL.
...
READ TABLE lt_phieuchi INDEX 1 into data(ls_company).
if sy-subrc = 0.
 lv_company  = ls_company-companycode.
    lo_chanky->get_zsign_data( EXPORTING i_tcode = 'ZPM04' i_zform = lv_company
                                  IMPORTING result    = lt_chanky ).
                                  ELSE.
               lo_chanky->get_zsign_data( EXPORTING i_tcode = 'ZPM04' i_zform = ''
                                  IMPORTING result    = lt_chanky ).
ENDIF.

  LOOP AT lt_phieuchi ASSIGNING FIELD-SYMBOL(<fs_phieuchi>).
      APPEND INITIAL LINE TO result ASSIGNING FIELD-SYMBOL(<fs_result>).

      " If it is create operation, then %cid needs to be used instead of %tky
      <fs_result>-%tky = <fs_phieuchi>-%tky.
