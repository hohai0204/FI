CLASS zcl_fa_zgl01_ex_n DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_FA_ZGL01_EX_N IMPLEMENTATION.
    ...
    INTO TABLE @DATA(lt_gl_compare).
        ...
        *
        ...
        LOOP AT lt_zfse2 INTO DATA(lw_zfse2) .
          ...
          READ TABLE lt_style_zsfe2 INTO DATA(lw_style) WITH KEY financial_statement = lw_zfse2-hierarchynode BINARY SEARCH.
          IF sy-subrc = 0.
        ...
        IF lines( lt_data ) > 0.
          ...
          LOOP AT lt_data_tmp INTO DATA(ls_row) FROM skip + 1 TO skip + top  . "#EC CI_NOORDER
            SHIFT ls_row-ma_so LEFT DELETING LEADING '0'.
